...
#define SRSRAN_NBIOT_EXPECT_MORE_SF                                            \
  -3 // returned when expecting more subframes for successful decoding
#define SRSRAN_NBIOT_UE_DL_FOUND_DCI                                           \
  -4 // returned when DCI for given RNTI was found
...
typedef struct SRSRAN_API {
  ...
  uint32_t sib1_sfn[4 * SIB1_NB_MAX_REP]; // there are 4 SIB1 TTIs in each hyper
                                          // frame
...
SRSRAN_API int srsran_nbiot_ue_dl_set_cell(srsran_nbiot_ue_dl_t *q,
                                           srsran_nbiot_cell_t cell);

SRSRAN_API int srsran_nbiot_ue_dl_decode_fft_estimate(srsran_nbiot_ue_dl_t *q,
                                                      uint32_t sf_idx,
                                                      bool is_dl_sf);

SRSRAN_API int srsran_nbiot_ue_dl_decode_estimate(srsran_nbiot_ue_dl_t *q,
                                                  uint32_t sf_idx);

SRSRAN_API int srsran_nbiot_ue_dl_cfg_grant(srsran_nbiot_ue_dl_t *q,
                                            srsran_ra_nbiot_dl_grant_t *grant,
                                            uint32_t sf_idx);

SRSRAN_API int srsran_nbiot_ue_dl_find_dl_dci(srsran_nbiot_ue_dl_t *q,
                                              uint32_t sf_idx, uint16_t rnti,
                                              srsran_dci_msg_t *dci_msg);

int srsran_nbiot_ue_dl_find_dl_dci_type_siprarnti(srsran_nbiot_ue_dl_t *q,
                                                  uint16_t rnti,
                                                  srsran_dci_msg_t *dci_msg);
...
SRSRAN_API int srsran_nbiot_ue_dl_find_ul_dci(srsran_nbiot_ue_dl_t *q,
                                              uint32_t tti, uint32_t rnti,
                                              srsran_dci_msg_t *dci_msg);

SRSRAN_API uint32_t srsran_nbiot_ue_dl_get_ncce(srsran_nbiot_ue_dl_t *q);

SRSRAN_API void srsran_nbiot_ue_dl_set_sample_offset(srsran_nbiot_ue_dl_t *q,
                                                     float sample_offset);

SRSRAN_API int srsran_nbiot_ue_dl_decode(srsran_nbiot_ue_dl_t *q, cf_t *input,
                                         uint8_t *data, uint32_t sfn,
                                         uint32_t sf_idx);

SRSRAN_API int srsran_nbiot_ue_dl_decode_npdcch(srsran_nbiot_ue_dl_t *q,
                                                cf_t *input, uint32_t sfn,
                                                uint32_t sf_idx, uint16_t rnti,
                                                srsran_dci_msg_t *dci_msg);

SRSRAN_API int srsran_nbiot_ue_dl_decode_npdsch(srsran_nbiot_ue_dl_t *q,
                                                cf_t *input, uint8_t *data,
                                                uint32_t sfn, uint32_t sf_idx,
                                                uint16_t rnti);

int srsran_nbiot_ue_dl_decode_npdsch_bcch(srsran_nbiot_ue_dl_t *q,
                                          uint8_t *data, uint32_t tti);

int srsran_nbiot_ue_dl_decode_npdsch_no_bcch(srsran_nbiot_ue_dl_t *q,
                                             uint8_t *data, uint32_t tti,
                                             uint16_t rnti);
...
SRSRAN_API void srsran_nbiot_ue_dl_set_rnti(srsran_nbiot_ue_dl_t *q,
                                            uint16_t rnti);

SRSRAN_API void srsran_nbiot_ue_dl_set_mib(srsran_nbiot_ue_dl_t *q,
                                           srsran_mib_nb_t mib);

SRSRAN_API void srsran_nbiot_ue_dl_decode_sib1(srsran_nbiot_ue_dl_t *q,
                                               uint32_t current_sfn);

SRSRAN_API void
srsran_nbiot_ue_dl_get_sib1_grant(srsran_nbiot_ue_dl_t *q, uint32_t sfn,
                                  srsran_ra_nbiot_dl_grant_t *grant);

SRSRAN_API void srsran_nbiot_ue_dl_decode_sib(srsran_nbiot_ue_dl_t *q,
                                              uint32_t hfn, uint32_t sfn,
                                              srsran_nbiot_si_type_t type,
                                              srsran_nbiot_si_params_t params);

SRSRAN_API void
srsran_nbiot_ue_dl_get_next_si_sfn(uint32_t current_hfn, uint32_t current_sfn,
                                   srsran_nbiot_si_params_t params,
                                   uint32_t *si_hfn, uint32_t *si_sfn);

SRSRAN_API void
srsran_nbiot_ue_dl_set_si_params(srsran_nbiot_ue_dl_t *q,
                                 srsran_nbiot_si_type_t type,
                                 srsran_nbiot_si_params_t params);

SRSRAN_API bool srsran_nbiot_ue_dl_is_sib1_sf(srsran_nbiot_ue_dl_t *q,
                                              uint32_t sfn, uint32_t sf_idx);

SRSRAN_API void srsran_nbiot_ue_dl_set_grant(srsran_nbiot_ue_dl_t *q,
                                             srsran_ra_nbiot_dl_grant_t *grant);
...
SRSRAN_API void
srsran_nbiot_ue_dl_check_grant(srsran_nbiot_ue_dl_t *q,
                               srsran_ra_nbiot_dl_grant_t *grant);

SRSRAN_API void srsran_nbiot_ue_dl_save_signal(srsran_nbiot_ue_dl_t *q,
                                               cf_t *input, uint32_t tti,
                                               uint32_t sf_idx);
