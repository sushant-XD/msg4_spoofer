...
typedef struct SRSRAN_API {
  ...
  uint8_t
      *allocated; // Allocated polar bit buffer, encoder input, decoder output
  cf_t *symbols;
...
/**
 * @brief Function for generating NR PDCCH candidate locations n_cce for a given
 * CORESET, search space, aggregation level and slot.
 *
 ...
 * @param search_space is the Search Space configuration provided from higher
 * layers
 ...
 * @param locations is the destination array with the possible candidate
 * locations n_cce
 */
int srsran_pdcch_nr_locations_coreset(
    const srsran_coreset_t *coreset, const srsran_search_space_t *search_space,
    uint16_t rnti, uint32_t aggregation_level, uint32_t slot_idx,
    uint32_t locations[SRSRAN_SEARCH_SPACE_MAX_NOF_CANDIDATES_NR]);

SRSRAN_API int
srsran_pdcch_nr_max_candidates_coreset(const srsran_coreset_t *coreset,
 ...
 * @param[out] rb_mask The resultant mask indicating the locations of PDCCH
 * payload
 * @return SRSRAN_SUCCESS if the provided parameters are valid, SRSRAN_ERROR
 * code otherwise
 */
SRSRAN_API int
srsran_pdcch_nr_cce_to_reg_mapping(const srsran_coreset_t *coreset,
                                   const srsran_dci_location_t *dci_location,
                                   bool rb_mask[SRSRAN_MAX_PRB_NR]);

SRSRAN_API int srsran_pdcch_nr_init_tx(srsran_pdcch_nr_t *q,
                                       const srsran_pdcch_nr_args_t *args);

SRSRAN_API int srsran_pdcch_nr_init_rx(srsran_pdcch_nr_t *q,
                                       const srsran_pdcch_nr_args_t *args);
 ...
 * @note This function shall not allocate, free memory or perform any heavy
 * computations
 ...
 * @return SRSRAN_SUCCESS if the configurations are valid, otherwise it returns
 * an SRSRAN_ERROR code
 */
SRSRAN_API int srsran_pdcch_nr_set_carrier(srsran_pdcch_nr_t *q,
                                           const srsran_carrier_nr_t *carrier,
                                           const srsran_coreset_t *coreset);

SRSRAN_API int srsran_pdcch_nr_encode(srsran_pdcch_nr_t *q,
                                      const srsran_dci_msg_nr_t *dci_msg,
                                      cf_t *slot_symbols);
 ...
 * @param[in,out] dci_msg Provides with the DCI message location, RNTI, RNTI
 * type and so on. Also, the message data buffer
 * @param[out] res Provides the PDCCH result information
 * @return SRSRAN_SUCCESS if the configurations are valid, otherwise it returns
 * an SRSRAN_ERROR code
 */
SRSRAN_API int srsran_pdcch_nr_decode(srsran_pdcch_nr_t *q, cf_t *slot_symbols,
                                      srsran_dmrs_pdcch_ce_t *ce,
                                      srsran_dci_msg_nr_t *dci_msg,
                                      srsran_pdcch_nr_res_t *res);

/**
 * @brief Stringifies NR PDCCH decoding information from the latest
 * encoded/decoded transmission
 *
