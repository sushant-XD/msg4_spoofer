...
/**
 * @brief Maximum number of NR DCI sizes the UE shall search for a given serving
 * cell
 */
...
typedef struct SRSRAN_API {
  ...
  bool monitor_common_0_0; ///< Set to true if Format 0_0 is monitored in common
                           ///< SS
  bool monitor_0_0_and_1_0; ///< Set to true if Format 0_0 is monitored in
                            ///< UE-specific SS
  bool monitor_0_1_and_1_1; ///< Set to true if Formats 0_1 and 1_1 are
                            ///< monitored in UE-specific SS
  ...
  uint32_t nof_rb_groups; ///< For allocation type 0, defined in TS 38.214
                          ///< clause 6.1.2.2.1

  /// Format 0_1 specific configuration (for PUSCH only)
  uint32_t nof_ul_bwp;      ///< Number of UL BWPs excluding the initial UL BWP,
                            ///< mentioned in the TS as N_BWP_RRC
  uint32_t nof_ul_time_res; ///< Number of dedicated PUSCH time domain resource
                            ///< assigment, set to 0 for default
  uint32_t nof_srs;         ///< Number of configured SRS resources
  uint32_t nof_ul_layers;   ///< Set to the maximum number of layers for PUSCH
  uint32_t pusch_nof_cbg; ///< determined by maxCodeBlockGroupsPerTransportBlock
                          ///< for PUSCH
  uint32_t report_trigger_size;    ///< determined by reportTriggerSize
  bool enable_transform_precoding; ///< Set to true if PUSCH transform precoding
                                   ///< is enabled
  bool dynamic_dual_harq_ack_codebook; ///< Set to true if HARQ-ACK codebook is
                                       ///< set to dynamic with 2 sub-codebooks
  bool pusch_tx_config_non_codebook;   ///< Set to true if PUSCH txConfig is set
                                       ///< to non-codebook
  bool pusch_ptrs; ///< Set to true if PT-RS are enabled for PUSCH transmission
  bool pusch_dynamic_betas; ///< Set to true if beta offsets operation is not
                            ///< semi-static
  ...
  uint32_t nof_dl_bwp;      ///< Number of DL BWPs excluding the initial UL BWP,
                            ///< mentioned in the TS as N_BWP_RRC
  uint32_t nof_dl_time_res; ///< Number of dedicated PDSCH time domain resource
                            ///< assigment
  uint32_t nof_aperiodic_zp; ///< Number of aperiodic ZP CSI-RS resource sets
                             ///< configured
  uint32_t pdsch_nof_cbg; ///< determined by maxCodeBlockGroupsPerTransportBlock
                          ///< for PDSCH
  uint32_t nof_dl_to_ul_ack;   ///< Number of entries in the dl-DataToUL-ACK
  bool pdsch_inter_prb_to_prb; ///< Set to true if interleaved VRB to PRB
                               ///< mapping is enabled
  bool pdsch_rm_pattern1;      ///< Set to true if rateMatchPatternGroup1 is
                               ///< configured
  bool pdsch_rm_pattern2;      ///< Set to true if rateMatchPatternGroup2 is
                               ///< configured
  bool pdsch_2cw; ///< Set to true if maxNrofCodeWordsScheduledByDCI is set to 2
                  ///< in any BWP
  ...
  bool pdsch_dynamic_bundling; ///< Set to true if prb-BundlingType is set to
                               ///< dynamicBundling
  srsran_resource_alloc_t pdsch_alloc_type; ///< PDSCH resource allocation type,
...
typedef struct SRSRAN_API {
  ...
  uint32_t
      dci_0_0_and_1_0_common_size; ///< DCI format 0_0 and 1_0 in common SS size
  ...
  uint32_t dci_0_0_and_1_0_ue_size; ///< DCI format 0_0 and 1_0 in UE-specific
                                    ///< SS size
...
typedef struct SRSRAN_API {
  ...
  uint32_t
      pucch_resource; ///< PUCCH resource indicator for HARQ feedback
                      ///< @note PUCCH resource is selected from
                      ///< PUCCH-ResourceSet if available, otherwise the UE
                      ///< shall pick a pucch-ResourceCommon from Table 9.2.1-1.
  uint32_t harq_feedback; ///< PDSCH-to-HARQ_feedback timing indicator
                          ///< @note harq_feedback for format 1_0 indicates the
                          ///< delay between the PDSCH reception and the UL
                          ///< transmission timing
                          ///< @note harq_feedback for format 1_1 is index of
                          ///< the delay indicated in DL data to UL ACK
  ...
  // Second TB (Optional)
  uint32_t
      mcs2; ///< Modulation and coding scheme for the second transport block
  ...
  uint32_t
      coreset0_bw; ///< CORESET0 size used for frequency resource allocation

} srsran_dci_dl_nr_t;

/**
 * @brief Describes an unpacked UL NR DCI message
 * @remark NR RAR UL DCI Described in TS 38.213 Table 8.2-1: Random Access
 * Response Grant Content field size
 */
typedef struct SRSRAN_API {
  // Context information
...
/**
 * @brief Set NR-DCI configuration for a given cell. This function will
 * pre-compute the DCI sizes, padding, truncation and so on from a given DCI
 * configuration.
 ...
 * @return SRSRAN_SUCCESS if the configuration is valid, SRSRAN_ERROR code
 * otherwise
 */
SRSRAN_API int srsran_dci_nr_set_cfg(srsran_dci_nr_t *dci,
                                     const srsran_dci_cfg_nr_t *cfg);
...
/**
 * @brief Indicates whether the provided DCI message format bit indicator
 * belongs to the a Dl DCI format (1_0 or 1_1) according according to the RNTI
 * type. If invalid, the DCI message is likely to be an UL DCI
 ...
 * @return SRSRAN_SUCCESS if provided arguments are valid, SRSRAN_ERROR code
 * otherwise
 */
SRSRAN_API int srsran_dci_nr_dl_pack(const srsran_dci_nr_t *q,
 ...
 * otherwise
 */
SRSRAN_API int srsran_dci_nr_dl_unpack(const srsran_dci_nr_t *q,
                                       srsran_dci_msg_nr_t *msg,
                                       srsran_dci_dl_nr_t *dci);

/**
 * @brief Packs an UL NR DCI into a DCI message
 * @param q NR DCI object with precomputed DCI parameters (not required for RAR
 * type, set to NULL)
 ...
 * otherwise
 */
SRSRAN_API int srsran_dci_nr_ul_pack(const srsran_dci_nr_t *q,
 ...
 * otherwise
 */
SRSRAN_API int srsran_dci_nr_ul_unpack(const srsran_dci_nr_t *q,
                                       srsran_dci_msg_nr_t *msg,
 ...
 */
SRSRAN_API uint32_t srsran_dci_ctx_to_str(const srsran_dci_ctx_t *ctx,
