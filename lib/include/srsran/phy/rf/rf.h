...
typedef struct {
  ...
  void (*srsran_rf_register_error_handler)(
      void *h, srsran_rf_error_handler_t error_handler, void *arg);
  ...
  int (*srsran_rf_recv_with_time)(void *h, void *data, uint32_t nsamples,
                                  bool blocking, time_t *secs,
                                  double *frac_secs);
  int (*srsran_rf_recv_with_time_multi)(void *h, void **data, uint32_t nsamples,
                                        bool blocking, time_t *secs,
                                        double *frac_secs);
  int (*srsran_rf_send_timed)(void *h, void *data, int nsamples, time_t secs,
                              double frac_secs, bool has_time_spec,
                              bool blocking, bool is_start_of_burst,
                              bool is_end_of_burst);
  int (*srsran_rf_send_timed_multi)(void *h, void **data, int nsamples,
...
SRSRAN_API int srsran_rf_open_multi(srsran_rf_t *h, char *args,
                                    uint32_t nof_channels);

SRSRAN_API int srsran_rf_open_devname(srsran_rf_t *h, const char *devname,
                                      char *args, uint32_t nof_channels);

/**
 * @brief Opens a file-based RF abstraction
 * @param[out] rf Device handle
 * @param[in] rx_files List of pre-opened FILE* for each RX channel; NULL to
 * disable
 * @param[in] tx_files List of pre-opened FILE* for each TX channel; NULL to
 * disable
 ...
 */
SRSRAN_API int srsran_rf_open_file(srsran_rf_t *rf, FILE **rx_files,
                                   FILE **tx_files, uint32_t nof_channels,
                                   uint32_t base_srate);

SRSRAN_API const char *srsran_rf_name(srsran_rf_t *h);

SRSRAN_API int srsran_rf_start_gain_thread(srsran_rf_t *rf,
                                           bool tx_gain_same_rx);
...
SRSRAN_API int srsran_rf_set_rx_gain_ch(srsran_rf_t *h, uint32_t ch,
                                        double gain);
...
SRSRAN_API void srsran_rf_register_error_handler(
    srsran_rf_t *h, srsran_rf_error_handler_t error_handler, void *arg);

SRSRAN_API double srsran_rf_set_rx_freq(srsran_rf_t *h, uint32_t ch,
                                        double freq);

SRSRAN_API int srsran_rf_recv(srsran_rf_t *h, void *data, uint32_t nsamples,
                              bool blocking);

SRSRAN_API int srsran_rf_recv_with_time(srsran_rf_t *h, void *data,
                                        uint32_t nsamples, bool blocking,
                                        time_t *secs, double *frac_secs);

SRSRAN_API int srsran_rf_recv_with_time_multi(srsran_rf_t *h, void **data,
                                              uint32_t nsamples, bool blocking,
                                              time_t *secs, double *frac_secs);
...
SRSRAN_API int srsran_rf_set_tx_gain_ch(srsran_rf_t *h, uint32_t ch,
                                        double gain);

SRSRAN_API double srsran_rf_set_tx_freq(srsran_rf_t *h, uint32_t ch,
                                        double freq);

SRSRAN_API void srsran_rf_get_time(srsran_rf_t *h, time_t *secs,
                                   double *frac_secs);

SRSRAN_API int srsran_rf_sync(srsran_rf_t *rf);

SRSRAN_API int srsran_rf_send(srsran_rf_t *h, void *data, uint32_t nsamples,
                              bool blocking);

SRSRAN_API int srsran_rf_send2(srsran_rf_t *h, void *data, uint32_t nsamples,
                               bool blocking, bool start_of_burst,
                               bool end_of_burst);

SRSRAN_API int srsran_rf_send(srsran_rf_t *h, void *data, uint32_t nsamples,
                              bool blocking);

SRSRAN_API int srsran_rf_send_timed(srsran_rf_t *h, void *data, int nsamples,
                                    time_t secs, double frac_secs);

SRSRAN_API int srsran_rf_send_timed2(srsran_rf_t *h, void *data, int nsamples,
                                     time_t secs, double frac_secs,
                                     bool is_start_of_burst,
                                     bool is_end_of_burst);

SRSRAN_API int srsran_rf_send_timed_multi(srsran_rf_t *rf, void **data,
                                          int nsamples, time_t secs,
                                          double frac_secs, bool blocking,
                                          bool is_start_of_burst,
                                          bool is_end_of_burst);

SRSRAN_API int srsran_rf_send_multi(srsran_rf_t *rf, void **data, int nsamples,
                                    bool blocking, bool is_start_of_burst,
                                    bool is_end_of_burst);
