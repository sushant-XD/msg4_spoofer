 ...
 */
 ...
 /**
  * @brief Maximum number of SSB positions in burst. Defined in TS 38.331
 ...
 typedef struct SRSRAN_API {
   double
       max_srate_hz; ///< Maximum sampling rate in Hz, set to zero to use default
   srsran_subcarrier_spacing_t min_scs; ///< Minimum subcarrier spacing
   bool enable_search;                  ///< Enables PSS/SSS blind search
   bool enable_measure;     ///< Enables PSS/SSS CSI measurements and frequency
                            ///< domain search
   ...
   float pbch_dmrs_thr; ///< NR-PBCH DMRS threshold for blind decoding, set to 0
                        ///< for default
 } srsran_ssb_args_t;
 ...
 typedef struct SRSRAN_API {
   ...
   srsran_ssb_pattern_t
       pattern; ///< SSB pattern as defined in TS 38.313 section 4.1 Cell search
   srsran_duplex_mode_t
       duplex_mode;         ///< Set to true if the spectrum is paired (FDD)
 ...
 typedef struct SRSRAN_API {
   ...
   uint32_t max_symbol_sz; ///< Maximum symbol size given the minimum supported
                           ///< SCS and sampling rate
   uint32_t max_corr_sz;   ///< Maximum correlation size
   uint32_t max_ssb_sz;    ///< Maximum SSB size in samples at the configured
                           ///< sampling rate
   uint32_t sf_sz;     ///< Current subframe size at the specified sampling rate
   uint32_t symbol_sz; ///< Current SSB symbol size (for the given base-band
                       ///< sampling rate)
   ...
   int32_t f_offset; ///< SSB integer frequency offset (multiple of SCS) between
                     ///< DC and the SSB center
   uint32_t cp_sz;   ///< CP length for the given symbol size
 
   /// Other parameters
   uint32_t
       l_first[SRSRAN_SSB_NOF_CANDIDATES]; ///< Start symbol for each SSB
                                           ///< candidate in half radio frame
   ...
   cf_t
       *pss_seq[SRSRAN_NOF_NID_2_NR]; ///< Possible frequency domain PSS for find
 } srsran_ssb_t;
 
 /**
  * @brief Describes an SSB search result
  * @note if pbch.crc is true, SSB transmission is found and decoded. Otherwise,
  * no SSB transmission has been decoded
  */
 typedef struct {
   uint32_t N_id;                 ///< Most suitable physical cell identifier
   uint32_t t_offset;             ///< Time offset in the input samples
   srsran_pbch_msg_nr_t pbch_msg; ///< Physical broadcast channel message of the
                                  ///< most suitable SSB candidate
  ...
  * @return SRSRAN_SUCCESS if the parameters are valid, SRSRAN_ERROR code
  * otherwise
  */
  ...
  * otherwise
  */
 SRSRAN_API int srsran_ssb_search(srsran_ssb_t *q, const cf_t *in,
                                  uint32_t nof_samples,
                                  srsran_ssb_search_res_t *res);
 
 /**
  * @brief Decides if the SSB object is configured and a given subframe is
  * configured for SSB transmission
  * @param q SSB object
  * @param sf_idx Subframe index within the radio frame
  * @return true if the SSB object is configured and SSB is transmitted, false
  * otherwise
  */
  ...
  * @return SRSRAN_SUCCES if inputs and data are correct, otherwise SRSRAN_ERROR
  * code
  */
 SRSRAN_API int srsran_ssb_put_grid(srsran_ssb_t *q, uint32_t N_id,
                                    const srsran_pbch_msg_nr_t *msg,
                                    cf_t *re_grid, uint32_t grid_bw_sc);
  ...
  * otherwise
  */
 SRSRAN_API int srsran_ssb_add(srsran_ssb_t *q, uint32_t N_id,
                               const srsran_pbch_msg_nr_t *msg, const cf_t *in,
                               cf_t *out);
  ...
  * otherwise
  */
 SRSRAN_API int srsran_ssb_decode_pbch(srsran_ssb_t *q, uint32_t N_id,
                                       uint32_t n_hf, uint32_t ssb_idx,
                                       const cf_t *grid,
  ...
  * otherwise
  */
 SRSRAN_API int srsran_ssb_decode_grid(srsran_ssb_t *q, uint32_t N_id,
                                       uint32_t n_hf, uint32_t ssb_idx,
                                       const cf_t *in, uint32_t grid_sz_rb,
                                       srsran_pbch_msg_nr_t *msg);
 
 /**
  * @brief Perform cell search and measurement
  * @note This function assumes the SSB transmission is aligned with the input
  * base-band signal
  * @param q SSB object
  ...
  * otherwise
  */
 SRSRAN_API int srsran_ssb_csi_search(srsran_ssb_t *q, const cf_t *in,
                                      uint32_t nof_samples, uint32_t *N_id,
  ...
  * otherwise
  */
 SRSRAN_API int srsran_ssb_csi_measure(srsran_ssb_t *q, uint32_t N_id,
                                       uint32_t ssb_idx, const cf_t *in,
                                       srsran_csi_trs_measurements_t *meas);
  ...
  * otherwise
  */
 SRSRAN_API int srsran_ssb_find(srsran_ssb_t *q, const cf_t *sf_buffer,
                                uint32_t N_id,
                                srsran_csi_trs_measurements_t *meas,
  ...
  * otherwise
  */
 SRSRAN_API int srsran_ssb_track(srsran_ssb_t *q, const cf_t *sf_buffer,
                                 uint32_t N_id, uint32_t ssb_idx, uint32_t n_hf,
                                 srsran_csi_trs_measurements_t *meas,
                                 srsran_pbch_msg_nr_t *pbch_msg);
 
 /**
  * @brief Calculates the subframe index within the radio frame of a given SSB
  * candidate for the SSB object
  * @param q SSB object
  * @param ssb_idx SSB candidate index
  * @param half_frame Indicates whether it is half frame
  * @return The subframe index
  */
 SRSRAN_API uint32_t srsran_ssb_candidate_sf_idx(const srsran_ssb_t *q,
                                                 uint32_t ssb_idx,
                                                 bool half_frame);
 
 /**
  * @brief Calculates the SSB offset within the subframe of a given SSB candidate
  * for the SSB object
  * @param q SSB object
  * @param ssb_idx SSB candidate index
  * @return The sample offset within the subframe
  */
 SRSRAN_API uint32_t srsran_ssb_candidate_sf_offset(const srsran_ssb_t *q,
                                                    uint32_t ssb_idx);
 
 SRSRAN_API uint32_t srsran_ssb_cfg_to_str(const srsran_ssb_cfg_t *cfg,
