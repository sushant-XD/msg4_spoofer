...
static int test(srsran_pdcch_nr_t *tx, srsran_pdcch_nr_t *rx, cf_t *grid,
                srsran_dmrs_pdcch_ce_t *ce, srsran_dci_msg_nr_t *dci_msg_tx) {
  // Encode PDCCH
  ...
  // Decode PDCCH
  TESTASSERT(srsran_pdcch_nr_decode(rx, grid, ce, &dci_msg_rx, &res) ==
             SRSRAN_SUCCESS);
...
int main(int argc, char **argv) {
  ...
  for (uint32_t frequency_resources = 1;
       ...
       frequency_resources = (fast_sweep) ? ((frequency_resources << 1U) | 1U)
                                          : (frequency_resources + 1)) {
    ...
    for (coreset.duration = SRSRAN_CORESET_DURATION_MIN;
         coreset.duration <= SRSRAN_CORESET_DURATION_MAX; coreset.duration++) {
      ...
      if (srsran_pdcch_nr_set_carrier(&pdcch_tx, &carrier, &coreset) <
          SRSRAN_SUCCESS) {
      ...
      for (uint32_t aggregation_level = 0;
        ...
        for (uint32_t slot_idx = 0;
          ...
          int n = srsran_pdcch_nr_locations_coreset(&coreset, &search_space,
                                                    rnti, aggregation_level,
          ...
          for (uint32_t ncce_idx = 0; ncce_idx < n; ncce_idx++) {
            ...
            for (uint32_t i = 0; i < dci_msg.nof_bits; i++) {
              dci_msg.payload[i] =
                  srsran_random_uniform_int_dist(rand_gen, 0, 1);
            }

            // Set channel estimate number of elements and set out-of-range
            // values to zero
            ...
            if (test(&pdcch_tx, &pdcch_rx, buffer, ce, &dci_msg) <
                SRSRAN_SUCCESS) {
              ERROR("test failed");
              goto clean_exit;
            }
