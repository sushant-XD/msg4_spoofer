...
static uint32_t coreset0_idx =
    0; // if ss_type=si coreset0 is used and this is the index
static uint32_t coreset_offset_rb = 0;
static bool interleaved_pdcch = false;
static uint32_t dl_arfcn =
    161200; // center of the NR carrier (default at 806e6 Hz)
static uint32_t ssb_arfcn =
    161290; // center of the SSB within the carrier (default at 806.45e6)
...
static void usage(char *prog) {
  ...
  printf("\t-c Coreset0 index (only used if SS type is common0 for SIB) "
         "[Default %d]\n",
         coreset0_idx);
...
static int work_ue_dl(srsran_ue_dl_nr_t *ue_dl, srsran_slot_cfg_t *slot) {
  ...
  int nof_found_dci =
      srsran_ue_dl_nr_find_dl_dci(ue_dl, slot, rnti, rnti_type, &dci_dl_rx, 1);
  ...
  for (uint32_t i = 0; i < ue_dl->pdcch_info_count; i++) {
    ...
    INFO("PDCCH: %s-rnti=0x%x, crst_id=%d, ss_type=%s, ncce=%d, al=%d, "
         "EPRE=%+.2f, RSRP=%+.2f, corr=%.3f; "
         "nof_bits=%d; crc=%s;",
         srsran_rnti_type_str_short(info->dci_ctx.rnti_type),
         info->dci_ctx.rnti, info->dci_ctx.coreset_id,
         srsran_ss_type_str(info->dci_ctx.ss_type), info->dci_ctx.location.ncce,
         info->dci_ctx.location.L, info->measure.epre_dBfs,
         info->measure.rsrp_dBfs, info->measure.norm_corr, info->nof_bits,
  ...
  if (rnti_type == srsran_rnti_type_ra) {
    ...
    pdsch_hl_cfg.common_time_ra[0].sliv = srsran_ra_type2_to_riv(
        SRSRAN_NSYMB_PER_SLOT_NR - 1, 1, SRSRAN_NSYMB_PER_SLOT_NR);
    pdsch_hl_cfg.nof_common_time_ra = 1;
  }
  if (srsran_ra_dl_dci_to_grant_nr(&carrier, slot, &pdsch_hl_cfg, &dci_dl_rx,
                                   &pdsch_cfg,
  ...
  // Decode PDSCH
  if (srsran_ue_dl_nr_decode_pdsch(ue_dl, slot, &pdsch_cfg, &pdsch_res) <
      SRSRAN_SUCCESS) {
    ERROR("Error decoding PDSCH search");
    return SRSRAN_ERROR;
  }
...
int main(int argc, char **argv) {
  ...
  // Open filesource
  if (srsran_filesource_init(&filesource, filename, SRSRAN_COMPLEX_FLOAT_BIN) <
      SRSRAN_SUCCESS) {
  ...
  if (rnti_type == srsran_rnti_type_si || rnti_type == srsran_rnti_type_ra) {
    ...
    double pointA_abs_freq_Hz =
        carrier.dl_center_frequency_hz -
        carrier.nof_prb * SRSRAN_NRE * SRSRAN_SUBC_SPACING_NR(carrier.scs) / 2;
    ...
    if (srsran_coreset_zero(carrier.pci, ssb_pointA_freq_offset_Hz, carrier.scs,
                            carrier.scs, coreset0_idx,
                            coreset) != SRSRAN_SUCCESS) {
      printf("Not possible to create CORESET Zero (ssb_scs=%s, pdcch_scs=%s, "
             "idx=%d)",
             srsran_subcarrier_spacing_to_str(carrier.scs),
             srsran_subcarrier_spacing_to_str(carrier.scs), coreset0_idx);
      return clean_exit(ret);
    }
    ...
    if (pdsch_time_ra_start >= 0 && pdsch_time_ra_length >= 0) {
      auto last_pdsch_symbol =
          (uint16_t)(pdsch_time_ra_start + pdsch_time_ra_length);

      if (last_pdsch_symbol > SRSRAN_NSYMB_PER_SLOT_NR) {
        ERROR("incorrect PDSCH start symbol or length provided");
        return clean_exit(ret);
      }
      uint32_t sliv = srsran_ra_nr_type1_riv(
          SRSRAN_NSYMB_PER_SLOT_NR, pdsch_time_ra_start, pdsch_time_ra_length);

      pdsch_hl_cfg.nof_dedicated_time_ra = 1;
      pdsch_hl_cfg.dedicated_time_ra[0].mapping_type =
  ...
  search_space->id = 0;
  search_space->coreset_id =
      (rnti_type == srsran_rnti_type_si || rnti_type == srsran_rnti_type_ra)
          ? 0
          : 1;
  ...
  for (uint32_t L = 0; L < SRSRAN_SEARCH_SPACE_NOF_AGGREGATION_LEVELS_NR; L++) {
    search_space->nof_candidates[L] =
        srsran_pdcch_nr_max_candidates_coreset(coreset, L);
  }
  ...
  if (srsran_filesource_read(&filesource, buffer[0], (int)ue_dl.fft->sf_sz) <
      SRSRAN_SUCCESS) {
