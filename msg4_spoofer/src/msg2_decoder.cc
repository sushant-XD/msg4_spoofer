...
MSG2Decoder::MSG2Decoder(uint32_t sample_rate_, uint32_t nof_prb_,
    ...
    : sample_rate(sample_rate_), nof_prb(nof_prb_), pci(pci_),
      carrier_freq(carrier_freq_), buffer(nullptr), ue_dl_initialized(false),
      softbuffer_initialized(false), owns_ue_dl(true) {
...
MSG2Decoder::MSG2Decoder(uint32_t sample_rate_, uint32_t nof_prb_,
                         uint32_t pci_, double carrier_freq_,
                         srsran_ue_dl_nr_t *external_ue_dl)
    : sample_rate(sample_rate_), nof_prb(nof_prb_), pci(pci_),
      carrier_freq(carrier_freq_), buffer(nullptr), ue_dl_initialized(true),
      softbuffer_initialized(false), owns_ue_dl(false) {
  // Use external UE DL instance
  memcpy(&ue_dl, external_ue_dl, sizeof(ue_dl));
  memset(&softbuffer_rx, 0, sizeof(softbuffer_rx));
...
MSG2Decoder::~MSG2Decoder() {
  ...
  if (ue_dl_initialized && owns_ue_dl) {
    srsran_ue_dl_nr_free(&ue_dl);
    ue_dl_initialized = false;
  }
  if (softbuffer_initialized) {
    srsran_softbuffer_rx_free(&softbuffer_rx);
    softbuffer_initialized = false;
  }
}
...
bool MSG2Decoder::init() {
  ...
  // Initialize UE DL only if we own it
  if (owns_ue_dl) {
    ...
    if (srsran_ue_dl_nr_init(&ue_dl, input_ptrs, &ue_dl_args) <
        SRSRAN_SUCCESS) {
      LOG_ERROR("Failed to initialize ue_dl_nr");
      return false;
    }
    ue_dl_initialized = true;
    ...
    if (srsran_ue_dl_nr_set_carrier(&ue_dl, &carrier) < SRSRAN_SUCCESS) {
      LOG_ERROR("Failed to set carrier");
      return false;
    }
  }
  ...
  if (srsran_softbuffer_rx_init_guru(
    ...
    LOG_ERROR("Failed to initialize softbuffer");
    return false;
  }
  softbuffer_initialized = true;

  // Calculate RA-RNTI list based on PRACH configuration
  calculate_ra_rnti_list();

  LOG_ERROR("MSG2 Decoder initialized with %ld RA-RNTIs to monitor",
            ra_rnti_list.size());
  return true;
}
...
std::vector<MSG2Result> MSG2Decoder::process_slot(cf_t *iq_samples,
                                                  uint32_t slot_idx) {
  ...
  LOG_INFO("Processing SFN=%d, Slot=%d (absolute slot %d)", sfn, slot_in_frame,
           slot_idx);
...
MSG2Decoder::process_slot_with_prach_slot(cf_t *iq_samples, uint32_t slot_idx,
                                          uint32_t prach_slot_in_frame) {
  std::vector<MSG2Result> results;

  // Validate PRACH slot
  if (prach_slot_in_frame % 2 != 0) {
    LOG_ERROR("Invalid PRACH slot %d (must be even: 0,2,4,6,8)",
              prach_slot_in_frame);
    return results;
  }
  ...
  LOG_ERROR("Processing SFN=%d, Slot=%d with specific RA-RNTI=0x%04x", sfn,
            slot_in_frame, ra_rnti);
  ...
  if (result.valid) {
    result.sfn = sfn;
    result.slot_in_frame = slot_in_frame;
    LOG_INFO("MSG2 decoded! SFN=%d, Slot=%d, RA-RNTI: 0x%04x, TC-RNTI:", sfn,
             slot_in_frame, result.ra_rnti);
    results.push_back(result);
  }
...
void MSG2Decoder::calculate_ra_rnti_list() {
  ...
  for (uint32_t t_id : prach_slots) {
    ...
    // logger.debug("PRACH slot %d -> RA-RNTI: %d (0x%04x)", t_id, ra_rnti,
    // ra_rnti);
  }
...
MSG2Result MSG2Decoder::try_decode_msg2(srsran_slot_cfg_t &slot_cfg,
                                        uint16_t ra_rnti) {
  MSG2Result result;
  ...
  int nof_dcis = srsran_ue_dl_nr_find_dl_dci(&ue_dl, &slot_cfg, ra_rnti,
                                             srsran_rnti_type_ra, dci_dl_list,
  ...
  for (int i = 0; i < nof_dcis; i++) {
    ...
    if (srsran_ra_dl_dci_to_grant_nr(&ue_dl.carrier, &slot_cfg, &phy_cfg.pdsch,
                                     &dci_dl_list[i], &pdsch_cfg,
    ...
    if (srsran_ue_dl_nr_decode_pdsch(&ue_dl, &slot_cfg, &pdsch_cfg,
...
bool MSG2Decoder::parse_rar_pdu(uint8_t *data, uint32_t len,
                                MSG2Result &result) {
  ...
  while (idx < len) {
    ...
    result.ul_grant = ((rar[1] & 0x0F) << 23) | (rar[2] << 15) | (rar[3] << 7) |
                      (rar[4] >> 1);
