...
int main(int argc, char *argv[]) {
  ...
  uint32_t window_size = ssb_sample_len;
  cf_t *window_buffer = srsran_vec_cf_malloc(ssb_sample_len);
  ...
  SIB1Processor sib1_processor(config.sample_rate, config.nof_prb,
                               config.ncellid, config.dl_freq);
  ...
  logger.info("MIB CORESET0 index: %u, SearchSpace0 index: %u", coreset0_idx,
              ss0_idx);

  // Calculate SSB to PointA frequency offset (critical for CORESET0)
  double pointA_abs_freq_Hz =
      config.dl_freq - config.nof_prb * SRSRAN_NRE *
                           SRSRAN_SUBC_SPACING_NR(config.scs_common) / 2;
  ...
  if (srsran_coreset_zero(config.ncellid, ssb_pointA_freq_offset_Hz,
                          config.scs_ssb, config.scs_common, coreset0_idx,
                          coreset0) < SRSRAN_SUCCESS) {
    logger.error("Failed to configure CORESET0 for SIB1 (coreset0_idx=%u)",
                 coreset0_idx);
    return EXIT_FAILURE;
  }

  logger.info(
      "CORESET0 configured: %u RBs, %u symbols, duration=%u, offset_rb=%u",
      coreset0->offset_rb, coreset0->duration, coreset0->duration,
      coreset0->offset_rb);
  logger.info("CORESET0 mapping: interleaved, bundle_size=%u, "
              "interleaver_size=%u, shift_index=%u",
              coreset0->reg_bundle_size, coreset0->interleaver_size,
              coreset0->shift_index);
  ...
  if (sib1_result.found) {
    ...
    logger.info("SIB1 JSON data length: %zu bytes",
                sib1_result.json_output.length());
    logger.info("=============================");
