...
SIB1Processor::SIB1Processor(double srate_hz, uint32_t nof_prb, uint32_t pci, double freq_hz)
...
SIB1SearchResult SIB1Processor::search_and_decode(
  ...
  LOG_INFO("Starting SIB1 search and decode (max_attempts=%u)...", max_attempts);
  ...
  for (uint32_t i = 0; i < max_attempts; ++i) {
    ...
    if (search_dci(ue_dl, phy_cfg, slot_cfg, SRSRAN_SIRNTI, 
                   srsran_rnti_type_si, sib1_dci)) {
      
      LOG_INFO("SIB1 DCI found in slot %u (attempt %u)! Attempting PDSCH decode...", slot_idx, i);
...
bool SIB1Processor::search_dci(srsran_ue_dl_nr_t& ue_dl,
  ...
  if (num_dci_dl > 0) {
    LOG_INFO("Found %d DCIs with RNTI 0x%x in slot %u", num_dci_dl, rnti, slot_cfg.idx);
    found_dci = dci_dl[0];
    return true;
  }
...
bool SIB1Processor::parse_sib1_asn1(const uint8_t* data,
                                    uint32_t data_len,
...
bool SIB1Processor::decode_sib1_pdsch(srsran_ue_dl_nr_t& ue_dl,
  ...
  if (srsran_ra_dl_dci_to_grant_nr(&ue_dl.carrier, &slot_cfg, nullptr,
                                   &sib1_dci, &pdsch_cfg, &pdsch_cfg.grant) < SRSRAN_SUCCESS) {
  ...
  // Initialize soft buffer
  srsran_softbuffer_rx_t softbuffer_rx = {};
  if (srsran_softbuffer_rx_init_guru(&softbuffer_rx, SRSRAN_SCH_NR_MAX_NOF_CB_LDPC, 
  ...
  // Decode PDSCH
  if (srsran_ue_dl_nr_decode_pdsch(&ue_dl, &slot_cfg, &pdsch_cfg, &pdsch_res) != SRSRAN_SUCCESS) {
    LOG_ERROR("Error in srsran_ue_dl_nr_decode_pdsch");
  ...
  if (!parse_sib1_asn1(sib1_data.data(), tbs_bytes, result.sib1_data, result.json_output)) {
